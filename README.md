...
`https://www.google.com/search?q=阿部寛&q=爆速ホームページ`となります。<br>
...
どちらもURLに記述されています。  
- 相違点<br>
クエリ文字列は、URLの末尾に「？」で始まり、「key=value」の形式でデータを伝達するため、主に検索機能として使用さます。　　
パス変数は、URLの一部をユーザーごとに変える変数として管理し、ユーザーに合わせて動的にサイトを管理する際に使用されます。
　　
　　
# HTTPメソッドとは
...
Webサイトを閲覧するにはクライアントからサーバーへ命令が送られます。この命令は`HTTPリクエスト`と呼ばれます。  
同様に、サーバーは受け取った命令からクライアントへ情報を返します。この情報を返すことを`HTTPレスポンス`と呼びます。  
...
Webサイトを閲覧するためのHTMLデータや画像データなどが含まれています。
